...
// Verify spot lock if token provided
if (!empty($lock_token)) {
    try {
        $stmt = $pdo->prepare("
...
// First, ensure parking location exists in database
try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_booking'])) {
    ...
    if ($vehicle_id <= 0) {
        ...
        try {
            ...
            // Verify the spot lock again before finalizing
            $stmt = $pdo->prepare("
                SELECT id FROM spot_locks 
                WHERE lock_token = ? 
                AND parking_location_id = ? 
            ...
            if (!$valid_lock) {
                ...
                // Check if spot is already booked (double check)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .booking-panel { max-width: 700px; margin: 60px auto; background: white; border-radius: 32px; padding: 35px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); }
        .booking-header { text-align: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #eef2ff; }
        .booking-header h2 { font-size: 1.8rem; color: #1e293b; margin-top: 10px; }
        .form-group { margin-bottom: 25px; }
        .form-group label { display: block; margin-bottom: 8px; font-weight: 600; color: #334155; }
        .form-group input, .form-group select, .form-group textarea { width: 100%; padding: 12px 16px; border: 2px solid #e2e8f0; border-radius: 12px; font-size: 1rem; font-family: 'Inter', sans-serif; }
        ...
        .btn-primary { width: 100%; background: linear-gradient(135deg, #2563eb, #1d4ed8); color: white; border: none; padding: 14px; border-radius: 48px; font-size: 1.1rem; font-weight: 600; cursor: pointer; transition: 0.2s; }
        ...
        .booking-details-card { background: linear-gradient(135deg, #f8fafc, #f1f5f9); border-radius: 20px; padding: 20px; margin: 25px 0; }
        .detail-row { display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid #e2e8f0; }
        .detail-row:last-child { border-bottom: none; }
        .detail-label { color: #64748b; font-weight: 500; }
        .detail-value { font-weight: 600; color: #1e293b; }
        .spot-badge { display: inline-block; background: #2563eb; color: white; padding: 4px 12px; border-radius: 20px; font-weight: 600; font-size: 0.9rem; }
        .vehicle-selector { background: #f8fafc; border-radius: 16px; padding: 15px; border: 2px solid #e2e8f0; }
        .vehicle-option { padding: 12px; margin: 8px 0; border-radius: 12px; background: white; border: 1px solid #e2e8f0; display: block; cursor: pointer; transition: 0.2s; }
        .vehicle-option:hover { background: #eff6ff; border-color: #2563eb; }
        .vehicle-option input { width: auto; margin-right: 12px; transform: scale(1.1); }
        .error-box { background: #fee2e2; color: #dc2626; padding: 14px; border-radius: 12px; margin-bottom: 20px; border-left: 4px solid #dc2626; }
        .error-box a { color: #2563eb; text-decoration: none; }
        .error-box a:hover { text-decoration: underline; }
        .amount-highlight { font-size: 1.5rem; font-weight: 800; color: #16a34a; }
        .add-vehicle-link { display: inline-block; margin-top: 12px; color: #2563eb; text-decoration: none; font-size: 0.9rem; }
        .warning-box { background: #fef3c7; padding: 12px; border-radius: 12px; margin-bottom: 20px; border-left: 4px solid #f59e0b; font-size: 0.85rem; }
        .lock-info { background: #e0e7ff; padding: 10px; border-radius: 8px; margin-top: 10px; font-size: 0.8rem; color: #4338ca; }
        @media (max-width: 768px) {
            .booking-panel { margin: 20px; padding: 20px; }
